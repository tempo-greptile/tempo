...
Tempo’s fee system provides several benefits for issuers:
...
In addition, when validators accept your token directly as their preferred fee token (so no AMM swap is required), your token earns **30 bps of each fee payment** as issuer fees. These issuer fees accumulate on the `FeeManager` and can be withdrawn by an address holding the token’s `FEE_CLAIM_ROLE` via the `FeeManager.claimTokenFees` function.
