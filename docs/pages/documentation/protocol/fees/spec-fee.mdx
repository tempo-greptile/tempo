...
On Tempo, users can pay gas fees in any [TIP-20](/documentation/protocol/tip20/spec) token whose currency is USD, as long as that stablecoin has sufficient liquidity on the enshrined [fee AMM](/documentation/protocol/fees/spec-fee-amm.mdx) against the token that the current validator wants to receive.
...
* Reserve `max_fee` of liquidity on the [fee AMM](/documentation/protocol/fees/spec-fee-amm) between the `fee_token` and the validator's preferred fee token. If there is insufficient liquidity, the transaction is invalid.
...
* Log a `Transfer` event from the user to the [fee manager contract](/documentation/protocol/fees/spec-fee-amm) for the net amount of the fee payment.

## Fee payer

Tempo supports *sponsored transactions* in which the `fee_payer` is a different address from the `tx.origin` of the transaction. This is supported by Tempo's [new transaction type](/documentation/protocol/transactions/spec-account-abstraction.mdx), which has a `fee_payer_signature` field.

If no `fee_payer_signature` is provided, then the `fee_payer` of the transaction is its sender (`tx.origin`).

If the `fee_payer_signature` field is set, then it is used to derive the `fee_payer` for the transaction, as described in the [transaction spec](/documentation/protocol/transactions/spec-account-abstraction.mdx).
...
* There must be sufficient liquidity on the [fee AMM](/documentation/protocol/fees/spec-fee-amm.mdx), as discussed in that specification.

If no preference is specified at the transaction, account, or contract level, the protocol falls back to [pathUSD](#pathusd).

### Transaction level

Tempo's [new transaction type](/documentation/protocol/transactions/spec-account-abstraction.mdx), allows transactions to specify a `fee_token` on the transaction. This overrides any preferences set at the account, contract, or validator level.
...
* If the transaction is not a [Tempo transaction](/documentation/protocol/transactions/spec-account-abstraction.mdx) *and* the transaction is a top-level call to the `setUserToken` function on the FeeManager, then the protocol checks the `token` argument to the function:
...
If the top-level call of a transaction is to a TIP-20 contract for which the currency is USD, or _all_ of the top-level calls of a native-AA transaction are to the same TIP-20 contract for which the currency is USD, that token is used as the user's fee token for that transaction (unless there is a preference specified at the [transaction](#transaction-level) or [account](#account-level) level).

### pathUSD

If no fee preference is set at the transaction, account, or contract level, the protocol falls back to [pathUSD](/documentation/protocol/exchange/pathUSD.mdx) as the user's fee token preference. 
...
#### 5. Fee swap queuing and issuer fees
...
If the user's fee token matches the validator's preference, no conversion is needed on the Fee AMM. In this case, the user pays the fee directly in the validator's preferred TIP-20 token, **99.7%** of the actual fee amount is credited to the validator, and the remaining **0.3% (30 bps)** of the fee is recorded as **issuer fees** for that TIP-20 token on the `FeeManager`. An address holding the token’s `FEE_CLAIM_ROLE` can later withdraw these issuer fees via `claimTokenFees(token, recipient)` on the `FeeManager` precompile.
...
For complete technical specifications on the Fee AMM mechanism, see the [Fee AMM Protocol Specification](/documentation/protocol/fees/spec-fee-amm).