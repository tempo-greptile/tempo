...
More information about Access Keys can be found in the [Account Keychain Specification](../../../specs/AccountKeychain.md).
...
**See the [Account Keychain Specification](../../../specs/AccountKeychain.md) for complete interface details, storage layout, and implementation.**
...
**Scope:** Only TIP20 `transfer()` and `transferFrom()` calls are tracked
...
1. Protocol intercepts `transfer(to, amount)` and `transferFrom(from, to, amount)` calls
2. Queries: `getRemainingLimit(account, keyId, token)`
3. Checks: `amount <= remaining_limit`
4. If check fails: reverts with `SpendingLimitExceeded`
5. If check passes: decrements the limit by `amount`
6. Updates are stored in precompile state
