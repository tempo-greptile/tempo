...
2. For each V1 validator (both active and inactive):
   - Create a V2 validator entry with:
     ...
     - `deletedAtHeight`: if V1 `active == true`, set to 0; if V1 `active == false`, set to `block.number`
...
- **All validators imported**: Both active and inactive V1 validators are
    imported; inactive ones have `deletedAtHeight = block.number`
...
23. **All validators imported**: Both V1 active and inactive validators are
    imported; inactive ones have `deletedAtHeight = block.number`
