...
interface IValidatorConfigV2 {
    ...
    ///      Does not set initialized flag - call initialize() after all validators are migrated.
    ...
    /// @param valIdx Index of the validator in V1 validators array
    function migrateValidator(uint64 valIdx) external;
    ...
    ///      Also copies nextDkgCeremony from V1 if not already set.
...
**`migrateValidator(valIdx)`**:
- Reads the validator from V1 at index `valIdx`
...
- Populates lookup maps (`validators[addr]`, `pubkeyToIndex[pubkey]`)
- Can be called multiple times to migrate validators one at a time
- Does NOT set `initialized` flag

**`initialize()`**:
- Copies `owner` from V1 to V2 (if not already set)
...
- **No tx gas limit risk**: Each migration tx handles only one validator
...
4. **Admin migrates validators** by calling `migrateValidator(valIdx)` for each validator
...
34. **initialize copies owner**: V2 `owner()` matches V1 after `initialize()`
...
## Gas Limits on Migration

Each validator imported during migration requires 11 SSTOREs (validator struct
fields, array push, pubkey mapping). With the per-validator `migrateValidator` approach,
this is no longer a blocking issue since each transaction only handles one validator.
