...
///
///      Note: Validators with `addedAtHeight == deletedAtHeight` are never considered active.
///      This is used during migration for V1 validators that were already inactive.
interface IValidatorConfigV2 {
    ...
    /// @dev Note: Validators with addedAtHeight == deletedAtHeight are never considered active.
    ...
    ///      Active V1 validators get addedAtHeight=allegroHeight and deletedAtHeight=0.
    ///      Inactive V1 validators get addedAtHeight=allegroHeight and deletedAtHeight=allegroHeight.
    /// @param allegroHeight The block height at which the Allegro hardfork activates.
    ///        Used to set addedAtHeight/deletedAtHeight for migrated validators.
    function initialize(uint64 allegroHeight) external;
...
| Aspect | V1 | V2 |
|--------|----|----|
| Status field | `active: bool` | `addedAtHeight: uint64` + `deletedAtHeight: uint64` |
...
- `deleteValidator(...)`
- `rotateValidator(...)`
- `initialize(allegroHeight)` (explicit initialization)
...
2. For each V1 validator (both active and inactive):
   - Create a V2 validator entry with:
     ...
     - `addedAtHeight`: set to `allegroHeight`
     - `deletedAtHeight`: set to `allegroHeight` if V1 `active == false`, otherwise 0
...
- **All validators imported**: Both active and inactive V1 validators are
    imported; inactive ones have `addedAtHeight == deletedAtHeight == allegroHeight`
...
29. **Write triggers init**: `addValidator`, `deleteValidator`, `rotateValidator`,
    `initialize(allegroHeight)` trigger initialization on first call; subsequent calls skip init
...
32. **addedAtHeight set correctly**: Imported validators have
    `addedAtHeight == allegroHeight` at initialization time
