...
interface IValidatorConfigV2 {
    ...
    ///          chainId, contractAddress, validatorAddress, inboundAddress, outboundAddress
    ///      ))
    /// @param validatorAddress The address of the new validator
    /// @param publicKey The new validator's Ed25519 communication public key
    /// @param inboundAddress The new validator's inbound address `<hostname|ip>:<port>` for incoming connections
    /// @param outboundAddress The new validator's outbound IP address `<ip>:<port>` for firewall whitelisting
    ...
    ///      populates V2 storage. Copies owner, validators, and nextDkgCeremony from V1.
    ///      Active V1 validators get addedAtHeight=hardforkHeight and deletedAtHeight=0.
    ///      Inactive V1 validators get addedAtHeight=hardforkHeight and deletedAtHeight=hardforkHeight.
    ///      The hardforkHeight is an immutable value set at contract deployment.
...
| Precompile address | `0xCCCC...0000` | `0xCCCC...0002` |
...
2. **Lazy initialization**: On first write, V2 automatically
...
V2 uses lazy initialization to automatically populate from V1 on first write.
The `initialized` bit (bit 255 of slot 0) tracks whether initialization has
occurred.

### Read Behavior (Before Initialization)

When `initialized == false`, read functions delegate entirely to V1 without
modifying V2 state:

- `getValidators()`: Calls `v1.getValidators()` and converts to V2 format
- `getActiveValidators()`: Calls `v1.getValidators()` and filters by `active == true`
- `validatorCount()`: Returns `v1.getValidators().length`
- `validatorByIndex(index)`: Returns `v1.getValidators()[index]` converted to V2 format
- `validatorByAddress(addr)`: Filters `v1.getValidators()` by address
- `validatorByPublicKey(pubkey)`: Filters `v1.getValidators()` by public key
- `getNextFullDkgCeremony()`: Calls `v1.getNextFullDkgCeremony()`

### Write Behavior (Triggers Initialization)

The following write functions trigger initialization if `initialized == false`:

- `addValidator(...)`
- `deleteValidator(...)`
- `rotateValidator(...)`
- `initialize()` (explicit initialization)

When any of these functions is called and `initialized == false`, the
following **Initialization Procedure** executes:

1. Read all validators from ValidatorConfig V1 at
   `0xCCCCCCCC00000000000000000000000000000000`
2. For each V1 validator (both active and inactive):
   - Create a V2 validator entry with:
     ...
     - `addedAtHeight`: set to `hardforkHeight`
     - `deletedAtHeight`: set to `hardforkHeight` if V1 `active == false`, otherwise 0
   - Add to `validatorsArray`
   - Populate lookup maps (`validators[addr]`, `pubkeyToIndex[pubkey]`)
3. Copy `owner` from V1 to V2
4. Copy `nextDkgCeremony` from V1 to V2
5. Set `initialized = true` (bit 255 of slot 0)
6. Proceed with the original write operation

```solidity
// V1 interface used during initialization and read delegation
...
- **Read-through before init**: Reads delegate to V1 with no state changes
- **Write-triggered init**: First write copies V1 state to V2, then executes
- **One-time**: Initialization executes exactly once; subsequent writes skip it
- **Atomic**: The entire initialization + write completes in a single transaction
...
- **All validators imported**: Both active and inactive V1 validators are
    imported; inactive ones have `addedAtHeight == deletedAtHeight == hardforkHeight`

## Timeline

```
Before Fork           Post-Fork (reads)         First V2 Write           After Initialization
     │                      │                          │                          │
     │  DKG reads V1        │  V2 reads proxy to V1    │  Init + write executes   │  V2 reads from V2
     │                      │  (no state change)       │  V2 populated from V1    │  initialized=1
     │                      │                          │                          │
─────┴──────────────────────┴──────────────────────────┴──────────────────────────┴───────────────►
                            │                          │
                      allegroTime               addValidator/etc
```
...
3. **At fork activation**: V2 reads proxy through to V1 (no state changes needed)
4. **On first write** (e.g., `addValidator`, `deleteValidator`): V2 automatically
   copies V1 state, sets `initialized = true`, then executes the write
5. **Post-init**: All reads/writes use V2 state directly
...
3. V1 Validator Config contract/precompile is not necessary in this flow
...
28. **Read delegation before init**: When `initialized == false`, `getValidators`,
    `getActiveValidators`, `validatorCount`, `validatorByIndex`, `validatorByAddress`,
    `validatorByPublicKey`, `getNextFullDkgCeremony` all delegate to V1 without
    modifying V2 state
29. **Write triggers init**: `addValidator`, `deleteValidator`, `rotateValidator`,
    `initialize()` trigger initialization on first call; subsequent calls skip init
30. **All validators imported on init**: Both V1 active and inactive validators are
    imported; inactive ones have `addedAtHeight == deletedAtHeight == hardforkHeight`
31. **DKG ceremony copied on init**: V2 `getNextFullDkgCeremony()` matches V1 after init
32. **addedAtHeight set correctly**: Imported validators have
    `addedAtHeight == hardforkHeight` at initialization time
33. **Owner copied on init**: V2 `owner()` matches V1 after initialization
34. **V1 to V2 format conversion**: Reads before init correctly convert V1 format
    (`active` bool) to V2 format (using `deletedAtHeight`)
35. **isInitialized returns correct value**: Returns false before init, true after

---

# Security Issues

## Gas Limits on Initialization

Each validator imported during initialization requires 11 SSTOREs (validator struct
fields, array push, pubkey mapping). This places an upper bound on the number of 
validators that can be migrated in a single transaction:

```
maxValidators = min(txGasLimit, blockGasLimit) / (11 * SSTORE_COST)
```

With current gas costs (~20,000 gas per cold SSTORE), this limits migration to
roughly `30,000,000 / (11 * 20,000) ≈ 136` validators per transaction at typical
block gas limits. Networks with more validators may need to consider batched
initialization or increased gas limits during the migration block.
