...
interface IValidatorConfigV2 {
    ...
    ///      Reverts if already initialized or if valConfigV1Idx != validatorsArray.length.
    ...
    /// @param valConfigV1Idx Index of the validator in V1 validators array (must equal current validatorsArray.length)
    /// @param valAddress The validator address to use in V2 (replaces the V1 validator address)
    function migrateValidator(uint64 valConfigV1Idx, address valAddress) external;
...
**`migrateValidator(valConfigV1Idx, valAddress)`**:
- Reverts if `isInitialized() == true`
- Reverts if `valConfigV1Idx != validatorsArray.length` (ensures sequential migration)
- On first call (when `validatorsArray.length == 0`), copies `owner` from V1 if V2 owner is `address(0)`
- Reads the validator from V1 at index `valConfigV1Idx`
- Creates a V2 validator entry with:
  - `publicKey`: copied from V1
  - `validatorAddress`: set to `valAddress` (provided parameter, not copied from V1)
  - `inboundAddress`: copied from V1
  - `outboundAddress`: copied from V1
  - `index`: set to `valConfigV1Idx`
...
- Populates lookup maps (`validators[valAddress]`, `pubkeyToIndex[pubkey]`)
...
- **CL reads V1 until flag flipped**: No read proxying in V2 precompile
...
4. **Admin migrates validators** by calling `migrateValidator(valConfigV1Idx, valAddress)` for each validator
   - One transaction per validator
   - Example: `migrateValidator(0, addr0)`, `migrateValidator(1, addr1)`, `migrateValidator(2, addr2)`, etc.
...
28. **migrateValidator imports validator**: Calling `migrateValidator(i, addr)`
    correctly imports validator at index i from V1 with the provided address
29. **migrateValidator reverts on duplicate**: Calling `migrateValidator(i, addr)` reverts
    if `i != validatorsArray.length`
29a. **migrateValidator reverts if initialized**: Calling `migrateValidator` reverts
...
32. **addedAtHeight set correctly**: Active validators have `addedAtHeight == 0`,
    inactive validators have `addedAtHeight == block.timestamp` at migration time
33. **initialize sets flag**: After `initialize()`, `isInitialized()` returns true
34. **migrateValidator copies owner**: V2 `owner()` matches V1 after first `migrateValidator` call
35. **initialize copies DKG ceremony**: V2 `getNextFullDkgCeremony()` matches V1
    after `initialize()`
36. **initialize owner only**: Non-owner cannot call `initialize`
37. **isInitialized returns correct value**: Returns false before initialize, true after
38. **Writes blocked before init**: `addValidator`, `deleteValidator`, `rotateValidator`
    revert if `isInitialized() == false` (only migration functions allowed)
