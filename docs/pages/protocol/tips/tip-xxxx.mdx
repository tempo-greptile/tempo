...
|------|-------|-------------|
| 0 (bit 0) | `initialized` | Whether V2 has been initialized from V1 (1 bit) |
| 0 (bits 96-255) | `owner` | Owner address (160 bits) |
...
The `initialized` bit occupies the lowest bit of slot 0 (Solidity uses big-endian
bit ordering). The `owner` address occupies the upper 160 bits.
...
11. **Initialized once**: The `initialized` bit (bit 0 of slot 0) can only
...
The `initialized` bit (bit 0 of slot 0) tracks whether initialization has
occurred.
...
3. Copy `nextDkgCeremony` from V1 to V2
4. Set `initialized = true` (bit 0 of slot 0)
...
- Allegro is active but V2 initialization fails (e.g., V1 read error)
- V2 validator set is empty after initialization (indicates V1 had no active
  validators)
...
22. **First read triggers init**: Calling `getValidators()` on uninitialized V2
    populates from V1
23. **First write triggers init**: Calling `addValidator()` on uninitialized V2
    populates from V1 before adding the new validator
24. **All triggering functions**: Each of `getValidators`, `getActiveValidators`,
    `validatorCount`, `validatorByIndex`, `validatorByAddress`,
    `validatorByPublicKey`, `addValidator` triggers initialization
25. **Initialization is idempotent**: Subsequent accesses do not re-initialize
26. **Active validators only**: V1 validators with `active == false` are not
    imported
27. **DKG ceremony copied**: V2 `getNextFullDkgCeremony()` matches V1
28. **addedAtHeight set correctly**: Imported validators have
    `addedAtHeight == block.number` at initialization time
29. **Initialized bit set**: After initialization, bit 0 of slot 0 is set
