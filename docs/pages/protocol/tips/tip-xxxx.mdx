...
|------|-------|-------------|
| 0 (bits 0-7) | `initialized` | Whether V2 has been initialized from V1 (1 byte, only bit 0 used) |
| 0 (bits 8-167) | `owner` | Owner address (160 bits) |
...
Solidity packs variables starting from the least significant byte. The
`initialized` flag occupies the first byte (bits 0-7), and the `owner` address
occupies the next 20 bytes (bits 8-167). Bits 168-255 are unused.
