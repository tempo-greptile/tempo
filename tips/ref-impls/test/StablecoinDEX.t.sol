...
contract StablecoinDEXTest is BaseTest {
    ...
    function test_MultiHopSwap_MaintainsSolvency() public {
        ...
        vm.prank(alice);
        uint128 amountOut = exchange.swapExactAmountIn(address(token1), address(token2), amountIn, 0);
        ...
        assertGt(
            bobPathUsdAfterSwap,
            bobPathUsdBeforeSwap,
            "hop2 should credit maker with pathUSD"
        );
    ...
    function test_MultiHopSwap_SolvencyInvariantHolds() public {
        ...
        assertGe(
            dexPathUsd,
            bobPathUsd,
            "SOLVENCY: DEX pathUSD balance covers bob's internal balance"
        );
