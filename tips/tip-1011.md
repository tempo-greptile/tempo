...
# TIP-xxxx: Subblocks v2 - Builder Delegation
...
**Delegation:**
- Only `validatorAddress` entries from `ValidatorConfig` with `active = true` can call `delegateToBuilder()`
...
- If a validator becomes inactive in `ValidatorConfig`:
  - Their delegation remains (gasWeight unchanged)
  - But their subblock gas contribution is excluded during block validation
- This simplifies registry logic while ensuring inactive validators don't contribute gas budget
...
- Replay protection via transaction hash + circular buffer
- Same validity window (30 seconds max)

The only difference from standard expiring nonces is the nonce key format, which routes the transaction to a specific builder.
...
A builder's gas budget is the sum of per-validator gas for all **active** validators delegating to them:

```
builder_gas_budget = per_validator_gas × count(active validators delegating to builder)
```

The `gasWeight` stored in the registry counts all delegating validators, but only active ones contribute to the budget during block validation.
...
- At least one active validator delegates to this builder
...
| ID | Invariant |
|----|-----------|
| **S1** | Delegation requirement: Only active validators can call `delegateToBuilder()` |
| **S2** | Gas weight consistency: `gasWeight[builder]` equals count of validators delegating to that builder |
...
12. **Inactive validator**: Validator becomes inactive, their gas contribution excluded from builder's budget
...
| Data | Current Location | TIP-xxxx Location |
|------|------------------|-------------------|
