...
2. Extract `builder_address` from nonce key (bytes 11-30)
3. Look up `config = SubblockRegistry.configOf(builder_address)`
...
**P2P peer set**: The DKG Manager reads relay node ed25519 pubkeys from SubblockRegistry for all builders with `gasWeight > 0` and adds them to the commonware p2p oracle. This allows relay nodes to join the authenticated network and deliver subblocks on behalf of their builder. Relay node authentication is handled by the p2p layer, not verified on-chain.
...
- **Bytes 11-30**: Builder's 20-byte ECDSA address
...
1. For each entry:
   ...
   b. Verify transactions in range have nonce key builder_address matching `entry.builder`
...
1. Extract `builder_address` from nonce key (bytes 11-30)
2. Look up `config = SubblockRegistry.configOf(builder_address)`
3. Use `config.feeRecipient` (required to be non-zero for active builders)
...
Relay nodes registered under builders with `gasWeight > 0` are added to the epoch-authorized peer set. When a builder's `gasWeight` drops to zero (all validators undelegated), their relay nodes are removed from the authorized set at the next epoch boundary.
...
2. For each 0x5b transaction:
   a. Extract builder_address from nonce key (bytes 11-30)
