...
1. For each entry (all lookups at parent block post-state):
   a. Verify ed25519 signature: `ed25519_verify(entry.signer, hash, entry.signature)`
   b. Verify signer is registered to builder: `p2pKeyOwner[entry.signer] == entry.builder`
   c. Verify transactions in range have nonce key builder_address matching `entry.builder`
   d. Verify `gasWeight[builder] > 0` (at least one validator in the set delegates to this builder)
...
- If sufficient AMM liquidity exists, swap to `feeToken[builder]`
...
4. Build SubBlockMetadata with version, builder, signer, tx_start_index,
...
2. For each 0x5b transaction:
   ...
   f. If builder has feeToken preference and AMM liquidity exists, swap
