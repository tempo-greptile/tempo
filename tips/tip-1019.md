...
struct SubBlockMetadataEntry {
    ...
    tx_count: u32,              // Number of transactions (must be > 0)
    signature: Bytes,           // ed25519 signature
}
...
2. Verify `entry.tx_count > 0` for each entry (empty subblocks are not valid)
3. Verify metadata entry ranges are non-overlapping, ordered, and within block bounds
4. Every `0x5b` transaction in the block MUST be covered by exactly one metadata entry
5. Metadata entries MUST NOT cover non-`0x5b` transactions
6. At most one metadata entry per builder address
7. If any `0x5b` transactions exist in the block, a SubBlockMetadata system transaction MUST be present
...
If a subblock transaction cannot pay fees (e.g., user emptied their account in the main block), the transaction becomes a **noop** but the **nonce is still incremented**. The block remains valid. This is a change from current behavior and should be noted during implementation.
...
| **S16** | Metadata required: If any `0x5b` transactions exist, a SubBlockMetadata system transaction must be present (and last) |
