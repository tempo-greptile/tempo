// TODO: desired tests
// round trips for all custom serde impls
//
// Possibly also snapshot tests

#[test]
fn boot_strapper_not_in_peers_is_rejected() {
    const INPUT: &str = r#"
signer = "0x81d35644dd13b5d712215023ab16615d9f8852c5a2fdfbd72dee06f538894b58"
share = "0x002ca4985d4850d2836b02a9597170ae3e122d4f858a11ed6d6447d1ca3ec3380d"
polynomial = ""
listen_port = 8000
metrics_port = 8001
storage_directory = "/Users/janis/dev/tempo/tempo-commonware/test_deployment/945fadcd1ea3bac97c86c2acbc539fce43219552d24aaa3188c3afc1df4d50a7/storage"
worker_threads = 3
bootstrappers = ["0xbaad106129bc215c1cca3760644914ed37ea91f1f1319999ce91ef2eaf51c826"]
message_backlog = 16384
mailbox_size = 16384
deque_size = 10
fee_recipient = "0x0000000000000000000000000000000000000000"

[peers]
0x945fadcd1ea3bac97c86c2acbc539fce43219552d24aaa3188c3afc1df4d50a7 = "127.0.0.1:8000"
0xbaad106129bc215c1cca3760644914ed37ea91f1f1319999ce91ef2eaf51c827 = "127.0.0.1:8002"
"#;

    let err = toml::from_str::<crate::Config>(INPUT)
        .expect_err("the single bootstrapper is not listed in peers, so this should give an error");

    // XXX: the typed error is passed through serde and toml, so we can only assert on the message
    assert_eq!(
        err.to_string().trim(),
        "bootstrapper `baad106129bc215c1cca3760644914ed37ea91f1f1319999ce91ef2eaf51c826` does not have an entry in the config's peers list"
    )
}
