...
mod tests {
    ...
    #[test]
    fn test_get_key_returns_correct_signature_type() -> eyre::Result<()> {
        let mut storage = HashMapStorageProvider::new(1);
        let account = Address::random();
        StorageCtx::enter(&mut storage, || {
            let mut keychain = AccountKeychain::new();
            ...
            for (sig_type, _expected_u8) in &sig_types {
                let key_id = Address::random();
                keychain.authorize_key(
                    account,
                    authorizeKeyCall {
                        keyId: key_id,
                        signatureType: sig_type.clone(),
                        expiry: u64::MAX,
                        enforceLimits: false,
                        limits: vec![],
                    },
                ...
                assert_eq!(
                    info.signatureType, *sig_type,
                    "get_key should return correct signature type for {sig_type:?}"
                );
                assert_eq!(info.keyId, key_id);
    ...
    #[test]
    fn test_get_key_revoked_returns_zero() -> eyre::Result<()> {
        let mut storage = HashMapStorageProvider::new(1);
        let account = Address::random();
        let key_id = Address::random();
        StorageCtx::enter(&mut storage, || {
            ...
            assert_eq!(info.keyId, Address::ZERO, "revoked key should return zero keyId");
            assert_eq!(info.expiry, 0, "revoked key should return zero expiry");
            assert!(info.isRevoked, "revoked key should have isRevoked=true");
    ...
    #[test]
    fn test_get_key_nonexistent_returns_zero() -> eyre::Result<()> {
        let mut storage = HashMapStorageProvider::new(1);
        StorageCtx::enter(&mut storage, || {
            let mut keychain = AccountKeychain::new();
            keychain.initialize()?;
            ...
            assert_eq!(info.keyId, Address::ZERO, "nonexistent key should return zero keyId");
            assert_eq!(info.expiry, 0, "nonexistent key should return zero expiry");
            assert!(!info.isRevoked, "nonexistent key should not be revoked");
    ...
    #[test]
    fn test_update_spending_limit_expired_key_rejected() -> eyre::Result<()> {
        let mut storage = HashMapStorageProvider::new(1);
        let account = Address::random();
        let key_id = Address::random();
        let token = Address::random();
        let now = std::time::SystemTime::now()
            ...
            .unwrap()
            .as_secs();
        let future_expiry = now + 60;

        // Authorize key with a near-future expiry
        StorageCtx::enter(&mut storage, || {
            let mut keychain = AccountKeychain::new();
            keychain.initialize()?;
            keychain.set_transaction_key(Address::ZERO)?;
        ...
        // Advance timestamp past expiry and try to update spending limit
        storage.set_timestamp(U256::from(future_expiry + 100));
        StorageCtx::enter(&mut storage, || {
            let mut keychain = AccountKeychain::new();
            keychain.set_transaction_key(Address::ZERO)?;
            ...
            let result = keychain.update_spending_limit(
                ...
                updateSpendingLimitCall {
                    keyId: key_id,
                    token,
                    newLimit: U256::from(100),
                },
            );
            assert!(result.is_err(), "updating spending limit on expired key should fail");
    ...
    #[test]
    fn test_update_spending_limit_enables_enforce_limits() -> eyre::Result<()> {
        let mut storage = HashMapStorageProvider::new(1);
        let account = Address::random();
        let key_id = Address::random();
        let token = Address::random();
        StorageCtx::enter(&mut storage, || {
            ...
            let info_before = keychain.get_key(getKeyCall {
                account,
                keyId: key_id,
            })?;
            assert!(!info_before.enforceLimits, "should start with enforce_limits=false");
            ...
            let info_after = keychain.get_key(getKeyCall {
                account,
                keyId: key_id,
            })?;
            assert!(info_after.enforceLimits, "update_spending_limit should enable enforce_limits");
    ...
    #[test]
    fn test_spending_limit_key_deterministic() {
        let account = Address::repeat_byte(0x01);
        let key_id = Address::repeat_byte(0x02);

        let key1 = AccountKeychain::spending_limit_key(account, key_id);
        let key2 = AccountKeychain::spending_limit_key(account, key_id);
        assert_eq!(key1, key2, "spending_limit_key should be deterministic");

        let key3 = AccountKeychain::spending_limit_key(key_id, account);
        assert_ne!(key1, key3, "different arg order should produce different key");
    }
}
