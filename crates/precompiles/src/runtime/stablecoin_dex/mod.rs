...
impl stablecoin_dex::IStablecoinDEX for StablecoinDEX {
    ...
    fn place(
        ...
        TIP20Token::from_address(non_escrow_token)?
            .ensure_transfer_authorized(msg_sender, self.address)?;
    ...
    fn place_flip(
        ...
        self.place_flip_internal(msg_sender, token, amount, is_bid, tick, flip_tick, false)
    }
    ...
    fn cancel_stale_order(&mut self, _msg_sender: Address, order_id: u128) -> Result<()> {
        let order = self.orders[order_id].read()?;
    ...
    fn get_tick_level(&self, base: Address, tick: i16, is_bid: bool) -> Result<PriceLevel> {
        ...
        let book_key = compute_book_key(base, quote);
        if is_bid {
            self.books[book_key].bids[tick].read().map(Into::into)
        } else {
            self.books[book_key].asks[tick].read().map(Into::into)
        }
    }
...
impl StablecoinDEX {
    ...
    /// Internal place_flip with internal_balance_only parameter
    #[allow(clippy::too_many_arguments)]
    fn place_flip_internal(
        &mut self,
        sender: Address,
        token: Address,
        amount: u128,
    ...
    fn fill_order(
        ...
        if order.is_flip() {
            ...
            let _ = self.place_flip_internal(
                order.maker(),
                orderbook.base,
                order.amount(),
                !order.is_bid(),
...
mod tests {
    ...
    fn test_settlement_rounding_favors_protocol() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            exchange.create_pair(Address::ZERO, base_token)?;
    ...
    fn test_place_order_below_minimum_amount() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            // Create the pair
            exchange
                .create_pair(Address::ZERO, base_token)
    ...
    fn test_place_flip_order_below_minimum_amount() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            let result = exchange.place_flip_internal(
                alice,
                base_token,
                below_minimum,
                true,
    ...
    fn test_place_flip_auto_creates_pair() -> Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            exchange.place_flip_internal(user, base_token, MIN_ORDER_AMOUNT, true, 0, 10, false)?;
    ...
    fn test_flip_order_execution() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            let flip_order_id = exchange
                .place_flip_internal(alice, base_token, amount, true, tick, flip_tick, false)
    ...
    fn test_find_trade_path_direct_pair() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            exchange
                .create_pair(Address::ZERO, token)
    ...
    fn test_find_trade_path_two_hop_siblings() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            // Create pairs first
            exchange.create_pair(Address::ZERO, usdc.address())?;
            exchange.create_pair(Address::ZERO, eurc.address())?;
    ...
    fn test_create_pair_invalid_currency() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            let result = exchange.create_pair(Address::ZERO, token_0.address());
            assert!(matches!(
                result,
    ...
    fn test_create_pair_rejects_non_tip20_base() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            let result = exchange.create_pair(Address::ZERO, non_tip20_address);
            assert!(matches!(
                result,
    ...
    fn test_decrement_balance_preserves_balance() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            exchange.create_pair(Address::ZERO, base_address)?;
    ...
    fn test_place_flip_order_immediately_active() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            let order_id = exchange.place_flip_internal(
                alice,
                base_token,
                min_order_amount,
                true,
    ...
    fn test_blacklisted_user_cannot_use_internal_balance() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            assert!(!registry.is_authorized(policy_id, alice)?);
    ...
    fn test_cancel_stale_order() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            exchange.create_pair(Address::ZERO, base.address())?;
            let order_id = exchange.place(alice, base.address(), MIN_ORDER_AMOUNT, false, 0)?;

            registry.modify_policy_blacklist(admin, policy_id, alice, true)?;

            exchange.cancel_stale_order(Address::ZERO, order_id)?;
    ...
    fn test_cancel_stale_not_stale() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            let result = exchange.cancel_stale_order(Address::ZERO, order_id);
            assert!(result.is_err());
            assert!(matches!(
                result.unwrap_err(),
    ...
    fn test_place_when_base_blacklisted() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            exchange.create_pair(Address::ZERO, base_addr)?;
