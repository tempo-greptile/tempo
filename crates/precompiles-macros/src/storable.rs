...
pub(crate) fn derive_space_impl(input: DeriveInput) -> syn::Result<TokenStream> {
    ...
    let ParsedStruct {
        ...
        mapping_names,
        ..
    } = setup_derive(&input, &field_infos)?;

    // Generate space handler struct for `DirectAddressMap` access
    let space_handler_struct = gen_space_handler_struct(
        strukt,
        &layout_fields,
        &mod_ident,
        &direct_names,
...
fn gen_space_handler_struct(
    ...
    quote! {
        ...
        impl crate::storage::Handler<#struct_name> for #space_handler_name {
            ...
            fn read(&self) -> crate::error::Result<#struct_name> {
                Ok(#struct_name {
                    #(#direct_names: crate::storage::Handler::read(&self.#direct_names)?),*,
                    #(#mapping_names: Default::default()),*
                })
            }
            ...
            #[inline]
            fn write(&mut self, value: #struct_name) -> crate::error::Result<()> {
                #(crate::storage::Handler::write(&mut self.#direct_names, value.#direct_names)?;)*
                Ok(())
            }

            #[inline]
            fn delete(&mut self) -> crate::error::Result<()> {
                #(crate::storage::Handler::delete(&mut self.#direct_names)?;)*
                Ok(())
            }
            ...
            fn t_read(&self) -> crate::error::Result<#struct_name> {
                Ok(#struct_name {
                    #(#direct_names: crate::storage::Handler::t_read(&self.#direct_names)?),*,
                    #(#mapping_names: Default::default()),*
                })
            }
            ...
            #[inline]
            fn t_write(&mut self, value: #struct_name) -> crate::error::Result<()> {
                #(crate::storage::Handler::t_write(&mut self.#direct_names, value.#direct_names)?;)*
                Ok(())
            }

            #[inline]
            fn t_delete(&mut self) -> crate::error::Result<()> {
                #(crate::storage::Handler::t_delete(&mut self.#direct_names)?;)*
                Ok(())
            }
        }
    }
